...
{
    class PalasAnalyzers : deviceAnalyzer
    {
        public PalasAnalyzers(DeviceType dvType, string devID)
        {
            //this.company = DeviceCompany.Palas;
            this.deviceType = dvType;
            this.deviceName = dvType.ToString();

            //_alarmValue = 0;
        }
        public PalasAnalyzers(DeviceType dvType, string devID, string devName, string ignorableErrors)
        {
        ...
        public enum PalasStatusType
        {
            UNINIT = 0, GOT02_SYNC = 1, GOT_ID = 2, GOT03_DATA = 3, GOT0A_CHKSUM1, GOT0A_CHKSUM2
        };

        PalasStatusType statusMsgRecv = PalasStatusType.UNINIT;

        public class MessagePalas
        {
            public string ID;
            public string Data;
        ...
        MessagePalas msgRecv = new MessagePalas();
        ...
        public override void parseByte(byte data)
        {
            ...
            if (statusMsgRecv < PalasStatusType.GOT03_DATA)
                msgRecv.checkSum ^= data;

            switch (statusMsgRecv)
            {
                case PalasStatusType.UNINIT:
                    if (data == 0x02)
                    {
                        statusMsgRecv++;
                    ...
                    break;
                case PalasStatusType.GOT02_SYNC:
                    if (data == 32)
                        statusMsgRecv++;
                    else
                    ...
                    break;
                case PalasStatusType.GOT_ID:
                    if (data == 0x03)
                        statusMsgRecv++;
                    else
                        msgRecv.Data += Convert.ToChar(data);
                    break;
                case PalasStatusType.GOT03_DATA:
                    msgRecv.StrChkSum += Convert.ToChar(data);
                    ++statusMsgRecv;
                    break;
                case PalasStatusType.GOT0A_CHKSUM1:
                    msgRecv.StrChkSum += Convert.ToChar(data);
                    Byte chs;

                    if (!Byte.TryParse(msgRecv.StrChkSum, NumberStyles.HexNumber, new CultureInfo("en-US"), out chs))
                        errorLog("[PalasAnalyzers3XX] [ParseByte] Cant Parse To Hex Data: " + msgRecv.StrChkSum);
            ...
            return;
        error:
        restart: statusMsgRecv = PalasStatusType.UNINIT;
            return;
        }
        ...
        public override void parseMessage()
        {
            ...
            try
            {
                ...
                {
                    polutions[k].concentration = tmpStrData.Length > 6 * k ? getFloatNumPalas(tmpStrData[6 * k + 1]) : - 10000f;
            ...
            {
                errorLog("[Error ParsePalasAnalyzers] - " + ex.Message);
            }
            finally
            {
                msgRecv = new MessagePalas();
            }
        ...
        private float getFloatNumPalas(string numStr)
        {
            try
            {
            ...
            {
                errorLog("[Problem Converting inPalasAnalyzers] for " + numStr + " - " + ex.Message);
            }
